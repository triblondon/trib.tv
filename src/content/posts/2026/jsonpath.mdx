...
Two path separators in a row with no path token in between is a *descendent selector* (at least in the [jsonpath-plus](https://github.com/JSONPath-Plus/JSONPath) parser) which allows the parser to navigate through *any number of levels* to reach the next matching token.  That's typically expressed as `..`, eg. `$..properties.*` (which matches children of the key `properties` *whereever it appears in the document*).
