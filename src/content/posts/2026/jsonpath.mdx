...
<JsonPathExample expr='$[paths][*][?(@property == "post" && @ && @.requestBody)]' source={jsonPathSample} />
...
3. Operations level: Select those with a key of "post" and a `requestBody` property in the value.  
...
Footnote: I was going to make the JSONPath analysis widget in [Codepen](https://codepen.io/), but then remembered that I build this site using [Astro](https://astro.build/) and so I could just make a component and Astro will remorselessly squash it into plain HTML+CSS at build time. Worked great. Still loving Astro.